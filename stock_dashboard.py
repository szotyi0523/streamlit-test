...
if data:
    ...
    if df is not None:
        ...
        with kpi3:
            st.metric(label="High Prices (last 30 days)", value=f"${df['High'][-30:].max():,.2f}")
        ...
        fig_close = px.bar(df, x=df.index, y="Volume", title=f"{stock_symbol.upper()} Trading Volume Over Time")
        st.plotly_chart(fig_close)
